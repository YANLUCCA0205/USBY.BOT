...
function parseMessage(message) {
  const lancamentoNotaPattern = /Lançamento de Cupom Fiscal[\s\S]*?Cod: (\d+)[\s\S]*?Nome: ([\w\s]+)[\s\S]*?Data: (\d{2}\/\d{2}\/\d{4})[\s\S]*?Coo: (\d+)[\s\S]*?Operadora: ([\w\s]+)[\s\S]*?Ecf: (\d+)[\s\S]*?Valor: (R\$ [\d,]+\.\d{2})/;
  const lancamentoNotinhaPattern = /Lançamento de Notinha Branca[\s\S]*?Cod: (\d+)[\s\S]*?Nome: ([\w\s]+)[\s\S]*?Data: (\d{2}\/\d{2}\/\d{4})[\s\S]*?Valor: (R\$ [\d,]+\.\d{2})/;
